...
SignupRoute.post("/", async (req,res) =>{
    ...
    if(user){
      return res.json({status:"NO",message : "User Already Exists, Please Signup with Different Credentials"})
    } else {
      if(token){
        try{
         ...
         if(verify){
            ...
            return res.status(201).json("New Employee Signedup")
                ...
                res.status(403).json({status : "NO", message :"Not Authorized !!"})
            }
         } else {
        ...
        const newUser = await Auth.create({email,name,password,age,gender,address,phone})
            return res.status(201).json({status : "OK", message : "New User Signedup"})
      }
    }
